def bubble_sort_2d(arr):
    ...
    m= len(arr[0]) # Number of columns in the 20 array; assumes all rows have equal length total elements Total number of elements in the 20 array
    total_elements = n * m # Total number of elements in the 2D array

    for i in range(total_elements -1):
        # Outer loop: goes through all elements in the 20 array

        for j in range(total_elements -1 -i):
            # Inner loop: goes through the elements, reducing the comparison range each time
        
            # Calculate current position in 20 terms
            row1 = j // n
            col1 = j % m
